...
export class EditTool implements Tool {
  ...
  onMouseDown(e: MouseEvent) {
    ...
    if (hover) {
      ...
      this.rectDragMeta = points
        ? this.getRectDragMeta(points, hover.polyIndex, hover.pointIndex)
        : null;
  ...
  onMouseMove(e: MouseEvent) {
    this.mousePos = this.getRelativePos(e);
    const allowInsert = checkModifier(
      e,
      this.context.options.keyMap?.insertPoint || ['Control', 'Meta'],
    );
  ...
  draw(ctx: CanvasRenderingContext2D) {
    ...
    if (this.ghostPoint) {
      ...
      ctx.beginPath();
      ctx.arc(
        this.ghostPoint.point.x,
        this.ghostPoint.point.y,
        this.context.options.pointRadius,
        0,
        Math.PI * 2,
      );
      ctx.fill();
